...
// ============================================
// Emails de venta B2C (tienda dropshipping)
// ============================================

type VentaB2cItemEmail = {
  nombre: string;
  cantidad: number;
  precioUnitEur: number | string;
  subtotalEur: number | string;
  marcaNombre?: string | null;
};

function eurFmt(v: number | string): string {
  const n = typeof v === 'string' ? Number(v) : v;
  if (!Number.isFinite(n)) return '—';
  return `${n.toFixed(2).replace(/\.00$/, '')} €`;
}

function filasItemsHtml(
  items: VentaB2cItemEmail[],
  opts: { mostrarMarca?: boolean; mostrarUnit?: boolean } = {},
): string {
  return items
    .map((it) => {
      const partes = [`${it.cantidad}× ${escapeHtml(it.nombre)}`];
      if (opts.mostrarMarca && it.marcaNombre) {
        partes.push(`<span style="color:${COLOR_STONE}">— ${escapeHtml(it.marcaNombre)}</span>`);
      }
      const right = opts.mostrarUnit
        ? `${eurFmt(it.precioUnitEur)}/u · ${eurFmt(it.subtotalEur)}`
        : eurFmt(it.subtotalEur);
      return `
        <tr>
          <td style="padding:8px 0;border-bottom:1px solid ${COLOR_LINE};font-size:14px;color:${COLOR_INK}">
            ${partes.join(' ')}
          </td>
          <td style="padding:8px 0;border-bottom:1px solid ${COLOR_LINE};font-size:14px;color:${COLOR_INK};text-align:right;white-space:nowrap">
            ${right}
          </td>
        </tr>`;
    })
...
export async function enviarConfirmacionVentaB2cCliente(params: {
  to: string;
  clienteNombre: string;
  numero: string;
  salonNombre: string;
  ...
  const subject = `Tu pedido ${params.numero} está confirmado`;
  const tiendaUrl = `${siteUrl}/s/${params.salonSlug}/tienda`;
  const cuerpoHtml = `
    <tr><td style="padding:8px 32px 16px">
      <h1 style="margin:0 0 12px;font-size:24px;font-weight:500;line-height:1.2;color:${COLOR_INK}">
        Pedido confirmado
      ...
      <p style="margin:0 0 16px;font-size:15px;line-height:1.55;color:${COLOR_INK}">
        Hola ${escapeHtml(params.clienteNombre.split(' ')[0] ?? params.clienteNombre)},
        hemos recibido tu pago correctamente. Tu pedido <strong>${escapeHtml(params.numero)}</strong>
        en <strong>${escapeHtml(params.salonNombre)}</strong> ya está en marcha.
      </p>

      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 16px">
        ${filasItemsHtml(params.items)}
        <tr>
          <td style="padding:12px 0 0;font-size:15px;font-weight:600;color:${COLOR_INK}">Total pagado</td>
          <td style="padding:12px 0 0;font-size:15px;font-weight:600;color:${COLOR_INK};text-align:right">${eurFmt(params.totalEur)}</td>
        </tr>
      </table>

      <p style="margin:16px 0 8px;font-size:13.5px;line-height:1.55;color:${COLOR_STONE}">
        <strong style="color:${COLOR_INK}">Dirección de envío:</strong><br/>
        ${escapeHtml(params.direccionEnvio)}
      </p>

      <p style="margin:16px 0 0;font-size:14px;line-height:1.55;color:${COLOR_INK}">
        Recibirás tu pedido en 3-5 días laborables. Si necesitas modificar la dirección
        o tienes cualquier duda, responde a este email.
      </p>

      <p style="margin:24px 0 0;font-size:12.5px;line-height:1.55;color:${COLOR_STONE}">
        Tienda del salón:
        <a href="${tiendaUrl}" style="color:${COLOR_TERRACOTTA};text-decoration:none">${escapeHtml(tiendaUrl)}</a>
      </p>
    </td></tr>
  `;
  return sendTemplate({
...
export async function enviarNotificacionVentaB2cSalon(params: {
  to: string;
  salonNombre: string;
  numero: string;
  clienteNombre: string;
  totalEur: number | string;
  comisionSalonEur: number | string;
  items: VentaB2cItemEmail[];
}): Promise<EmailResult> {
  const subject = `Nueva venta ${params.numero} en tu tienda`;
  const panelUrl = `${siteUrl}/panel`;
  const cuerpoHtml = `
    <tr><td style="padding:8px 32px 16px">
      <h1 style="margin:0 0 12px;font-size:24px;font-weight:500;line-height:1.2;color:${COLOR_INK}">
        Has vendido en tu tienda
      </h1>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.55;color:${COLOR_INK}">
        Hola ${escapeHtml(params.salonNombre)}, un cliente acaba de pagar el pedido
        <strong>${escapeHtml(params.numero)}</strong> en tu tienda online.
      </p>

      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 16px">
        ${filasItemsHtml(params.items)}
        <tr>
          <td style="padding:12px 0 0;font-size:14px;color:${COLOR_STONE}">Total cobrado al cliente</td>
          <td style="padding:12px 0 0;font-size:14px;color:${COLOR_STONE};text-align:right">${eurFmt(params.totalEur)}</td>
        </tr>
        <tr>
          <td style="padding:4px 0 0;font-size:15px;font-weight:600;color:${COLOR_INK}">Tu comisión</td>
          <td style="padding:4px 0 0;font-size:15px;font-weight:600;color:${COLOR_INK};text-align:right">${eurFmt(params.comisionSalonEur)}</td>
        </tr>
      </table>

      <p style="margin:16px 0 0;font-size:14px;line-height:1.55;color:${COLOR_INK}">
        Tu comisión se ha transferido automáticamente a tu cuenta Stripe Connect.
        Estará disponible en tu banco según tu calendario de payouts de Stripe.
      </p>

      <p style="margin:16px 0 8px;font-size:13.5px;line-height:1.55;color:${COLOR_STONE}">
        Nosotros nos encargamos del envío al cliente
        (${escapeHtml(params.clienteNombre)}). Tú no tienes que hacer nada.
      </p>

      <div style="margin:16px 0 0">${botonHtml('Ver pedidos', panelUrl)}</div>
    </td></tr>
  `;
  return sendTemplate({
    to: params.to,
...
export async function enviarVentaB2cAdminTramitar(params: {
  to: string;
  numero: string;
  salonNombre: string;
  clienteNombre: string;
  ...
  totalEur: number | string;
  comisionSalonEur: number | string;
  costeMarcaEur: number | string;
}): Promise<EmailResult> {
  const subject = `[TRAMITAR] Pedido ${params.numero} — ${params.salonNombre}`;
  const adminUrl = `${siteUrl.replace('://', '://admin.')}/ventas-b2c`;
  const margenBruto =
    Number(params.totalEur || 0) -
    Number(params.comisionSalonEur || 0) -
    Number(params.costeMarcaEur || 0);
  const cuerpoHtml = `
    <tr><td style="padding:8px 32px 16px">
      <h1 style="margin:0 0 12px;font-size:22px;font-weight:500;line-height:1.2;color:${COLOR_INK}">
        Tramitar pedido a la marca
      </h1>
      <p style="margin:0 0 16px;font-size:14px;line-height:1.55;color:${COLOR_INK}">
        Venta <strong>${escapeHtml(params.numero)}</strong> pagada en
        <strong>${escapeHtml(params.salonNombre)}</strong>. Acción manual:
        pasar el pedido a la marca para que envíe directo al cliente.
      </p>

      <h2 style="margin:24px 0 8px;font-size:14px;text-transform:uppercase;letter-spacing:1px;color:${COLOR_STONE}">
        Items a pedir
      </h2>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 16px">
        ${filasItemsHtml(params.items, { mostrarMarca: true, mostrarUnit: true })}
      </table>

      <h2 style="margin:24px 0 8px;font-size:14px;text-transform:uppercase;letter-spacing:1px;color:${COLOR_STONE}">
        Envío
      </h2>
      <p style="margin:0 0 4px;font-size:14px;color:${COLOR_INK}">
        <strong>${escapeHtml(params.clienteNombre)}</strong>
      </p>
      <p style="margin:0 0 4px;font-size:14px;color:${COLOR_INK};white-space:pre-wrap">${escapeHtml(params.direccionEnvio)}</p>
      <p style="margin:0 0 4px;font-size:13px;color:${COLOR_STONE}">
        ${escapeHtml(params.clienteEmail)}${params.clienteTelefono ? ` · ${escapeHtml(params.clienteTelefono)}` : ''}
      </p>

      <h2 style="margin:24px 0 8px;font-size:14px;text-transform:uppercase;letter-spacing:1px;color:${COLOR_STONE}">
        Resumen económico
      </h2>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:0 0 16px;font-size:13.5px;color:${COLOR_INK}">
        <tr><td style="padding:2px 0;color:${COLOR_STONE}">Cobrado al cliente</td><td style="padding:2px 0;text-align:right">${eurFmt(params.totalEur)}</td></tr>
        <tr><td style="padding:2px 0;color:${COLOR_STONE}">– Comisión al salón</td><td style="padding:2px 0;text-align:right">${eurFmt(params.comisionSalonEur)}</td></tr>
        <tr><td style="padding:2px 0;color:${COLOR_STONE}">– Coste marca (a pagar)</td><td style="padding:2px 0;text-align:right">${eurFmt(params.costeMarcaEur)}</td></tr>
        <tr><td style="padding:6px 0 0;font-weight:600">= Margen bruto Rogotech</td><td style="padding:6px 0 0;font-weight:600;text-align:right">${eurFmt(margenBruto)}</td></tr>
      </table>

      <div style="margin:24px 0 0">${botonSecundarioHtml('Abrir en admin', adminUrl)}</div>
    </td></tr>
  `;
  return sendTemplate({
    to: params.to,
