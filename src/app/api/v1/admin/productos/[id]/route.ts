...
import { categoriasMarca, productos } from '@/lib/db/schema';
...
const PatchBody = z.object({
  ...
  tipo_distribucion: z.enum(TiposDistribucion).optional(),
...
export async function PATCH(
  ...
  if (d.categoria_marca_id) {
    const [prod] = await db
      .select({ marcaId: productos.marcaId })
      .from(productos)
  ...
  if (d.categoria_marca_id !== undefined) update.categoriaMarcaId = d.categoria_marca_id;
  if (d.tipo_distribucion !== undefined) update.tipoDistribucion = d.tipo_distribucion;
